...
if ($nv_Request->isset_request('gid,exclude', 'post')) {
	...
	if (empty($in_groups)) {
		//Nếu không còn thuộc nhóm nào thì mặc định vào nhóm 4 (users). #issue1
		$db->query('UPDATE ' . NV_MOD_TABLE . " SET group_id = 4, in_groups='" . implode(',', $in_groups) . "', last_update=" . NV_CURRENTTIME . ' WHERE userid=' . $uid);
	} else {
