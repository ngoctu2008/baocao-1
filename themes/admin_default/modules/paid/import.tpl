            ...
            success : function(json) {
                if (json.error) {
                    ...
                    $.each(json.data, function(index, value) {
                        console.log(value)
                        $('#table-check tbody, #table-import tbody').append('<tr id="row-' + index + '"><td>' + value[0] + '</td><td>' + value[1] + '</td><td></td><td>' + value[2] + '</td><td class="status"></td></tr>');
                    });
                    total = json.total;
                    count_error = 0;
                    nv_users_readline();
